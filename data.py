...
def dataset_from_csv(df, cat_features, T):
    y = {}
    X_num = {}
    X_cat = {} if len(cat_features) else None
    ...
    if X_cat is not None:
        X_cat["train"] = df[cat_features].to_numpy().astype(str)
    X_num["train"] = df.drop(cat_features, axis=1).to_numpy().astype(float)

    y_info = {'policy': 'default'}
    task_type = None
    n_classes = 0
    dataset = Dataset(X_num, X_cat, y, y_info, task_type, n_classes)
    return transform_dataset(dataset, T, False)
...
def transform_dataset(dataset, transformations, return_transforms = False):
